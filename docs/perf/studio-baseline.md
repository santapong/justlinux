...
Measured with `hypr-claude-studio --bench 20` (real machine state: 5 claude
...
| `window_active` (every 2 s, **every** instance) | 4.7 ms fork × N | **stat()** every 500 ms | hooks write `$XDG_RUNTIME_DIR/claude-studio.active`; tmux asked only every 20 s to reconcile |
...
Re-measure: `hypr-claude-studio --bench 20`, then the 30 s tick script in
`bin/studio-idle.py`.
