...
if command -v cargo >/dev/null 2>&1 && [ -d rust ]; then
    if (cd rust && cargo build --release -p hypr-office2d -p hypr-pet -p hypr-cardhost -p hypr-appdock -p hypr-viz >/dev/null 2>&1); then
        ...
        echo "installed: ~/.local/bin/{hypr-office2d,hypr-pet,hypr-cardhost,hypr-appdock,hypr-viz} (built from rust/)"
    else
