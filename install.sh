...
if command -v cargo >/dev/null 2>&1 && [ -d rust ]; then
    if (cd rust && cargo build --release -p hypr-office2d -p hypr-pet -p hypr-cardhost -p hypr-appdock -p hypr-viz -p hypr-claude-studio -p hypr-docker >/dev/null 2>&1); then
