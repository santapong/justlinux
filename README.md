...
| `bin/hypr-appdock` | one auto-hiding dock per monitor (touch the bottom screen edge to reveal; `dock_<MON>_autohide=off` pins one monitor's dock open, e.g. `dock_DP_1_autohide=off`, global `dock_autohide` is the fallback; `dock_show_on_empty=off` also hides it on an empty workspace instead of keeping it shown), per-monitor pins — plus the **smart top bar**: `bar_smart=on` makes waybar itself auto-hide with top-edge dwell reveal, `ALT+B` pin, and a fade animation |
