...
| `hypr-kanban` | `ALT+CTRL+K` | **Habitica board** — three tabs, because Habitica has three kinds of task: **To-Dos** as a real board you **drag cards** across (To do · Doing · Done), **Dailies** (which recur, so "done" means done *today*), and **Habits** (a ＋/− you press, with nothing to finish). "Doing" is not a state Habitica has — it is a `doing` **tag**, so it shows on the phone and the website too, and it is only created the first time you move something there. `[` `]` move a card from the keyboard, `1 2 3` switch tabs. Keys live in `~/.config/hyprdesk/secrets.json` (0600), never in this repo |
